...
@CommandLine.Command(name = "ArvandDynamique",

    version = "ArvandDynamique 1.0",
...
public class ArvandDynamique extends AbstractPlanner<ADLProblem> {
    ...
    private static final Logger LOGGER = LogManager.getLogger(ArvandDynamique.class.getName());
...
public Node MonteCarloRandomWalk(Node s, ADLProblem problem){
        ...
        for(int i = 0; i < NUM_WALK; i++){
            ...
            this.h.add(hmin);
            if (progress(i+1) > acceptableProgress(i+1)) {
                return smin;
            }
            if(counter > 300){
                init_len_walk *= 1.5;
...
public Plan solve(final ADLProblem problem) {
     ...
     * The main method of the <code>ArvandDynamique</code> planner.
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            final ArvandDynamique planner = new ArvandDynamique();
...
javac -d classes -cp lib/pddl4j-4.0.0.jar src/fr/uga/pddl4j/tutorial/asp/Node.java src/fr/uga/pddl4j/tutorial/asp/ASP.java
